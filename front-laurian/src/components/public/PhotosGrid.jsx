...
function PhotosGrid() {
  ...
  const fetchImages = async () => {
    ...
    const filterImages = randomImages.filter((image) => image.status === "publié")
  ...
  return (
    <section className="main-container photos-section homepage-section">
            ...
            image.status === "publié" && (
              <>
                <ImageListItem key={index} className="image-card">
                  ...
                  />
                  <button
                    className={`vote-btn btn ${remainingVotes === 0 ? "disabled" : ""
      ...
      <Stack spacing={2}>
        <Pagination count={Math.ceil(images.length / 10)} color="secondary" page={currentPage} onChange={(event, page) => setCurrentPage(page)} />
      </Stack>
    </section>
  );
}
